...
getgenv().funcs = {
    ...
    loadstring = function(...) loadstring(...)() end,
    ...
    tohex = function() if typeof(color) == "Color3" then local current = math.floor(color.r * 255) * 256 ^ 2 + math.floor(color.g * 255) * 256 + math.floor(color.b * 255) local final = "" local hexChar = {"A", "B", "C", "D", "E", "F"} repeat local remainder = current % 16 local char = tostring(remainder) if remainder >= 10 then char = hexChar[1 + remainder - 10] end current = math.floor(current / 16) final = final..char until current <= 0 return "#"..string.reverse(final) else return error("Expected Color3 at function \"ToHex\".\nGot "..typeof(color), 2) end end,
